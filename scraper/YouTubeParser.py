...
class YouTubeParser(SeleniumParser):
    ...
    def parse_video(self, url: str) -> dict:
        ...
        video['channel_name'] = self.driver.find_element(By.XPATH, "//div[@class='style-scope ytd-channel-name']/div").text
        ...
        try:
            description = self.driver.find_element(By.XPATH, '//div[@id="description"]//ytd-text-inline-expander//yt'
                                                             '-attributed-string').find_element(By.TAG_NAME,
                                                                                                'span').text
        except:
            description = ''
        ...
        try:
            WebDriverWait(self.driver, 5).until(EC.element_to_be_clickable((By.ID, 'main')))
    ...
    def save_videos(self, channel, offset_url: str = None):
        ...
        if len(urls):
            ...
            with open(f"./data/youtube/{channel}.csv", "w", encoding='utf8', newline='') as file:
